...
app.post("/api/createmovie", (req, res) => {
  ...
  try {
    if (moviename && mil) {
      db.query(
        "insert into tbl_movie (name, mil) values (?,?) ",
        [moviename, mil],
        (err, resp, field) => {
          if (resp) {
...
app.get("/api/getallmovie", (req, res) => {
  try {
    db.query("select * from tbl_movie", [], (err, data, fil) => {
      if (data && data[0]) {
...
app.get("/api/getmoviebyid", (req, res) => {
  ...
  try {
    if (movieid) {
      db.query(
        "select * from tbl_movie where id = ?",
        [movieid],
        (err, data, fil) => {
          if (data && data[0]) {
            for (let i = 0; i < data.length; i++) {
...
app.put("/api/updatemovie", (req, res) => {
  ...
  try {
    if (id && name && mil) {
      db.query(
        "update tbl_movie set name = ?, mil = ? where id = ?",
...
app.delete("/api/deletemoviebyid", (req, res) => {
  ...
  try {
    if (id) {
      db.query(
        "delete from tbl_movie where id = ?",
        [parseInt(id)],
        (err, resp, fill) => {
          if (resp) {
            return res.status(200).json({
